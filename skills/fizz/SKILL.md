...
- Otherwise, default to `{MODE} = "automatic"` so the workflow works headlessly.

Every subsequent instruction referencing `{MODE}` must substitute the resolved value. Do NOT switch modes mid-run.

## Delegation

Use the current model. When a subagent facility exists, delegate within its concurrency limit; otherwise execute each role sequentially in the parent agent. `{AGENT_MODEL}` in legacy role templates means the current model and never requires a model-selection prompt.
...
After the banner, print the resolved mode. Explicit `--guided` pauses at documented checkpoints; every other invocation uses automatic mode.
...
If `{PROJECT_ROOT}/x-ray/x-ray.md` does NOT exist, run the bundled **x-ray Acquisition Protocol** below.

### x-ray Acquisition Protocol

1. Read `../x-ray/SKILL.md` relative to this skill and execute it against `{PROJECT_ROOT}`.
2. Verify that `{PROJECT_ROOT}/x-ray/x-ray.md` was produced.
3. If the bundled workflow cannot run, read `{SKILL_PATH}/agents/protocol-analyzer.md` and execute those instructions with the current agent or an available subagent. It must write `{PROJECT_ROOT}/{META_DIR}/protocol-understanding.md`.

Do not install skills, clone repositories, or fetch executable instructions during this workflow.
...
## Step 9: Generate Invariants (5 Discovery Roles + Synthesizer + 2 Implementers)

Skip this step only when the user passed `--no-invariants`.

This step uses **5 specialized discovery roles**, each applying a different invariant discovery approach drawn from 50+ real DeFi bugs caught by fuzzers.
...
### Step 9b: Execute 5 Discovery Roles

Run the roles concurrently when the runtime supports subagents; otherwise run them sequentially with the current agent.
...
Read each role file, replace `{INVARIANT_CONTEXT}` and `{FILE_PATHS}` with actual values, and execute it with the current model.
...
### Step 9d: Implement Properties (2 Roles)
...
After validation succeeds, execute the Report Writer role. Read `{SKILL_PATH}/agents/report-writer.md`, replace `{SKILL_PATH}` with the actual `{SKILL_PATH}`, `{PROJECT_ROOT}` with the actual `{PROJECT_ROOT}`, `{META_DIR}` with the actual `{META_DIR}`, and `{SUITE_DIR}` with the actual `{SUITE_DIR}`, then run it with the current model.
