...
**Execution**: Use the current model; run as a subagent when available or inline with the current agent otherwise.
...
2. `{PROJECT_ROOT}/PROPERTIES.md` — English-language spec with stable IDs and checkboxes (consumed by implementers and by the `/skill:fizz-convert` command).
...
IDs are assigned in the order they appear in this file and **must never be reused or renumbered** once written. The `/skill:fizz-convert` command and the implementers identify properties by these IDs.
...
> of three states:
> - `[ ]` pending — not yet implemented; `/skill:fizz-convert` will pick it up
> - `[x]` implemented — Solidity exists in Properties.sol / handlers
> - `[-]` skipped / manual — `/skill:fizz-convert` will NOT touch it. Use this for TODO stubs,
...
> them `[ ]` to have `/skill:fizz-convert` implement them, or `[-]` to leave them as a spec
> note only.
>
