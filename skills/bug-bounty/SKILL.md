...
description: Bug bounty workflow — recon (subdomain enumeration, asset discovery, HackerOne scope, source audit), pre-hunt learning, vulnerability hunting (IDOR, SSRF, XSS, auth bypass, CSRF, race conditions, SQLi, XXE, file upload, business logic, GraphQL, HTTP smuggling, cache poisoning, OAuth, OIDC, SSTI, subdomain takeover, cloud misconfig, ATO chains, LLM/AI), LLM/AI testing (prompt injection, ASCII smuggling, ASI01-ASI10), bug chaining (IDOR→auth bypass, SSRF→cloud metadata, XSS→ATO), bypass tables, language-specific grep, and reporting (7-Question Gate, CVSS 3.1, PoC generation, submission checklist). Use for any bug bounty task. 中文触发词：漏洞赏金、安全测试、渗透测试、漏洞挖掘、XSS测试、SQL注入、SSRF、安全审计、漏洞报告
---
