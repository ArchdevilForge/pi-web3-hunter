...
Track these three phases with the runtime's planning facility when one is available; otherwise keep the checklist in the working response:
...
- When Step 1's parallel batch returns, mark Phase 1 `completed` and Phase 2 `in_progress`.
- When Step 2 (including 2b–2g) finishes, mark Phase 2 `completed` and Phase 3 `in_progress`.
...
**Immediately after**, run the following concurrently when the runtime supports it, otherwise sequentially:

**0. Version check** (foreground):
- Read the local `VERSION` file from `$SKILL_DIR/VERSION`
- Do not fetch or execute remote update content during an audit.
...
- **Path B (>5 docs OR any doc >300 lines):** Use one available subagent, or the current agent when subagents are unavailable, to read all doc files and return a structured extraction (max 200 lines). Prompt:
  ```
  ...
  ```
  Run this work concurrently when supported. Its output feeds Step 3 report writing.
...
Run independent calls concurrently when supported. Proceed to Step 2 after their outputs are available.

## Step 2: Read Source Files + Entry Point Scan

Run independent reads and scans concurrently when the runtime supports it; otherwise run them sequentially.
...
### Path B: >20 source files

**Tier 1 — Small files (≤120 lines):** Batch into single Bash `cat` call.

**Tier 2 — Large files (>120 lines):** Group by subsystem. Use up to five available subagents, or the current agent sequentially, with at most about 10 files per group. Prompt:
```
