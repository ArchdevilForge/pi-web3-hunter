...
description: Reference for 26 web2 bug classes with root causes, detection patterns, bypass tables, and paid examples. Covers IDOR, auth bypass, XSS, SSRF (11 IP bypasses), SQLi, business logic, race conditions, OAuth/OIDC, file upload, GraphQL, LLM/AI (ASI01-ASI10), API misconfig, ATO, SSTI, subdomain takeover, cloud misconfig, HTTP smuggling, cache poisoning, MFA, SAML, error disclosure, CSS injection, LFI/RCE, deserialization, and dependency confusion. Use when hunting a specific vuln class or studying what makes bugs pay.
---
...
**WAF bypass for SSRF**: If WAF blocks `127.0.0.1`/`169.254.169.254`, try `2130706433` (decimal), `0x7f000001` (hex), `[::1]` (IPv6), `[::ffff:127.0.0.1]` (IPv4-mapped), `127.0.0.1.nip.io` (DNS rebind), or `127。0。0。1` (full-width period U+3002). Run payload through `"${HOME}/.agents/bughunter/bbhunt-tool" waf_encoder.py "<payload>" --class generic`.
...
**WAF bypass for file upload**: Run `"${HOME}/.agents/bughunter/bbhunt-tool" multipart_mutator.py --file shell.aspx --field file` for 10 parser-confusion variants (boundary simplification, double-boundary case-insensitive confusion, charset=utf-16le part encoding, null-byte in boundary, Content-Disposition sub-param injection, per-part image/jpeg Content-Type). Combine with polyglot (GIF89a magic bytes + PHP payload). RFC 2231 filename: `filename*=utf-8''shell.php`. MIME Base64: `filename="=?utf-8?b?c2hlbGwucGhw?="`.
