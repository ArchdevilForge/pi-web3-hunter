...
**Turn 1 — Discover.** Print the banner, then perform these checks concurrently when supported:
...
Do not fetch remote versions or installers during an audit. Use the current model. If a subagent facility exists, use it within its concurrency limit; otherwise execute the specialty passes sequentially in the parent agent.

**Turn 1b — Execution lane.** Use the current model. If the runtime exposes subagents, use them within its concurrency limit; otherwise execute every specialty pass sequentially in the parent agent.
...
**Turn 3a — Run all 12 specialty passes.** Use available subagents up to the runtime's concurrency limit, or execute the bundles sequentially in the parent agent. Wait for every pass before Turn 3b. Do not poll or sleep while delegated work is running.
...
5. **Auto-clean.** After output, resolve `{bundle_dir}` and verify it is the exact `.audit-*` directory created by this run under the project root, then remove only that directory. Preserve it when verification fails.
